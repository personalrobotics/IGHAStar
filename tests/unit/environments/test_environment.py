...
class EnvironmentTester:
            ...
            self.test_config = {
                "experiment_info_default": {
                    ...
                    "epsilon": [
                        ...
                        0.2,
                        5.0,
                    ],  # ate, cte, heading, vel for goal region
                    "tolerance": 0.5,
                    "max_level": 5,
                    "division_factor": 2.0,
    ...
    def load_test_maps(self):
        ...
        if self.env_type == "simple":
            ...
            self.valid_map = torch.full(
                (map_size, map_size), 255.0, dtype=torch.float32
            )
            # All invalid map (black pixels = 0)
            self.invalid_map = torch.full(
                (map_size, map_size), 0.0, dtype=torch.float32
            )
        else:
    ...
    def compile_test_environment(self):
        ...
        if self.env_type == "simple":
            ...
            cuda_path = (
                ...
                / "Environments"
                / "src"
                / "{}.cu".format(self.env_type)
            )
            sources = [str(pathlib.Path(__file__).parent / "test.cpp"), str(cuda_path)]
        ...
        header_path = BASE_DIR / "src" / "Environments" / "include"
        ...
        self.test_kernel = load(
            name="{}_{}_test".format(
                self.env_type, "cpu" if not cuda_available else "gpu"
            ),
            sources=sources,
    ...
    def setup_test_conditions(self):
        ...
        if self.env_type == "simple":
            ...
            self.test_pose_tensor = torch.tensor(
                [5.0, 5.0, 0.0, 0.0], dtype=torch.float32
            )
            ...
            self.close_goal_tensor = torch.tensor(
                [5.01, 5.01, 0.01, 0.0], dtype=torch.float32
            )
            self.far_goal_tensor = torch.tensor(
                [1.0, 1.0, 3.14, 0.0], dtype=torch.float32
            )
            ...
            # Map is 100x100 pixels with 0.1 resolution = 10x10 meters, so coordinates must be [0, 10)
            self.test_pose_tensor = torch.tensor(
                [5.0, 5.0, 0.0, 3.0], dtype=torch.float32
            )
            ...
            self.close_goal_tensor = torch.tensor(
                [5.01, 5.01, 0.01, 3.01], dtype=torch.float32
            )
            self.far_goal_tensor = torch.tensor(
                [1.0, 1.0, 3.14, 2.0], dtype=torch.float32
            )
...
def main():
    ...
    parser.add_argument(
        "--env",
        choices=["kinematic", "kinodynamic", "simple"],
        default="kinematic",
