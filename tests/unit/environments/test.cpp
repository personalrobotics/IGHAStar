...
public:
  ...
  ~EnvironmentTester() = default;

  // Test 1: Validity checking
  int test_validity(torch::Tensor world, torch::Tensor start_tensor,
  ...
  // Test 3: Successor generation - ensure at least one successor differs from
  // start
  ...
  bool test_goal_reaching_close(torch::Tensor world, torch::Tensor start_tensor,
  ...
  bool test_goal_reaching_far(torch::Tensor world, torch::Tensor start_tensor,
  ...
  bool test_heuristic_distance(torch::Tensor start_tensor,
    ...
    if (heuristic_cost < 0.0f) {
      std::cout << "✗ Heuristic returned negative value: " << heuristic_cost
                << std::endl;
      return false;
    }
    ...
    if (distance < 0.0f) {
      std::cout << "✗ Distance returned negative value: " << distance
                << std::endl;
      return false;
    }
    ...
    if (same_heuristic > 1e-3) {
      std::cout << "✗ Heuristic to same position too large: " << same_heuristic
                << std::endl;
      return false;
    }
    ...
    if (same_distance > 1e-6) {
      std::cout << "✗ Distance to same position not zero: " << same_distance
                << std::endl;
      return false;
    }

    std::cout << "✓ Heuristic and distance calculations work correctly"
              << std::endl;
    return true;
  }

  // Run all tests
  bool run_all_tests(torch::Tensor valid_map, torch::Tensor invalid_map,
                     torch::Tensor test_pose_tensor, torch::Tensor start_tensor,
                     torch::Tensor goal_tensor, torch::Tensor close_goal_tensor,
                     torch::Tensor far_goal_tensor) {
    ...
    if (invalid_result != 0) {
      std::cout << "✗ Invalid map test failed: expected 0, got "
                << invalid_result << std::endl;
      all_passed = false;
    } else {
      std::cout << "✓ All positions correctly identified as invalid"
                << std::endl;
    }
    ...
    if (valid_result != 2) {
      std::cout << "✗ Valid map test failed: expected 2, got " << valid_result
                << std::endl;
      all_passed = false;
    } else {
    ...
    all_passed &=
        test_successor_generation(valid_map, start_tensor, goal_tensor);
    all_passed &=
        test_goal_reaching_close(valid_map, start_tensor, close_goal_tensor);
    all_passed &=
        test_goal_reaching_far(valid_map, start_tensor, far_goal_tensor);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  py::class_<EnvironmentTester>(m, "EnvironmentTester")
      ...
      .def("test_successor_generation",
           &EnvironmentTester::test_successor_generation)
      .def("test_goal_reaching_close",
           &EnvironmentTester::test_goal_reaching_close)
      .def("test_goal_reaching_far", &EnvironmentTester::test_goal_reaching_far)
      .def("test_heuristic_distance",
           &EnvironmentTester::test_heuristic_distance)
