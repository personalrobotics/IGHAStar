...
constexpr int hash_dims = 3; // because we don't differentiate in the velocity dimension
...
public:
    ...
    ~Environment() {
        if (h_costmap) delete[] h_costmap;
        if (h_heightmap) delete[] h_heightmap;
    ...
    void check_validity(float *start, float *goal, bool *result) {
        ...
        check_validity_launcher_cpu(
            h_costmap, map_size_px, map_res,
            states,
