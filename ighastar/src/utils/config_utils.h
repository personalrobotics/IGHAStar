...
// Get a nested config dict, checking both root level and under
// experiment_info_default Returns (dict, found) pair - if not found, returns
// empty dict with found=false
inline std::pair<py::dict, bool> get_config_dict(const py::dict &config,
                                                 const char *key) {
