...
struct SamplingConfig {
  ...
  std::vector<int>
      angular_dims; // Which dimensions are angular (for SE2), empty if none
  int num_samples;
...
inline void interpolate_pose(const float *start, const float *end, float alpha,
...
//
// For SE2: samples in the local frame of center_pose using
// cross-track/along-track errors For R_N: samples using simple Gaussian
// perturbations
template <typename ValidityChecker>
std::vector<std::vector<float>>
sample_states_around_pose(const float *center_pose,
  ...
  for (int i = 0; i < config.num_samples; i++) {
    ...
    if (config.min_bounds && config.max_bounds) {
      for (int d = 0; d < config.n_dims; d++) {
        if (!is_angular_dim(
                d, config.angular_dims)) { // Don't clamp angular dimensions
          sampled_pose[d] = std::clamp(sampled_pose[d], config.min_bounds[d],
                                       config.max_bounds[d]);
        }
      }
    }
...
generate_intermediate_states(const float *start_pose, const float *end_pose,
                             int n_dims, const std::vector<int> &angular_dims,
                             int timesteps) {
...
inline std::vector<std::vector<float>>
generate_perturbation_cache(const float *scale_per_dim, int n_dims,
...
// Generates interior points at t = i/num_interpolation_points for i
// = 1..num_interpolation_points-1 (endpoints at t=0 and t=1 are skipped since
// they are the input poses themselves)
template <typename ValidityChecker>
bool check_interpolation_validity(
    const float *pose1, const float *pose2, int n_dims,
    const std::vector<int> &angular_dims, int num_interpolation_points,
  ...
  // Generate interior points (skip endpoints i=0 and
  // i=num_interpolation_points)
  for (int i = 1; i < num_interpolation_points; i++) {
    float t =
        static_cast<float>(i) / static_cast<float>(num_interpolation_points);
...
// Compute distance between two poses (Euclidean, ignoring angular wrapping for
// now)
inline float compute_distance(const float *pose1, const float *pose2,
                              int n_dims) {
  float sum_sq = 0.0f;
  for (int d = 0; d < n_dims; d++) {
...
inline float compute_se2_distance(const float *pose1, const float *pose2,
                                  int n_dims,
                                  const std::vector<int> &angular_dims,
...
// NodeFactory signature: (float* pose, float* intermediate_poses, float
// g_value, float f_value) -> NodeType HeuristicFn signature: (float* pose1,
// float* pose2) -> float
template <typename NodeType, typename ValidityChecker, typename HeuristicFn,
          typename NodeFactory>
std::vector<NodeType>
sample_and_create_nodes(const float *center_pose, const float *goal_pose,
                        const SamplingConfig &config, int timesteps,
                        ValidityChecker &&validity_checker,
                        HeuristicFn &&heuristic, NodeFactory &&node_factory) {

  // Step 1: Sample valid poses
  auto valid_poses = sample_states_around_pose(
      center_pose, config, std::forward<ValidityChecker>(validity_checker));
  ...
  for (auto &sampled_pose : valid_poses) {
    ...
    auto intermediates = generate_intermediate_states(
        center_pose, sampled_pose.data(), config.n_dims, config.angular_dims,
        timesteps);
    ...
    // Create node using factory (factory handles parent's g addition
    // internally)
    auto node = node_factory(sampled_pose.data(), intermediate_poses.data(),
                             g_value, f_value);
...
//   NodeFactory: (float* pose, float* intermediates, float g, float f) ->
//   NodeType NearMeetChecker: (const NodeType&) -> bool (returns true if valid)
//
// Returns vector of valid sampled nodes
template <typename NodeType, typename ValidityChecker, typename HeuristicFn,
          typename NodeFactory, typename NearMeetChecker>
std::vector<NodeType>
sample_goal_region(const float *start_pose, const float *goal_pose,
                   const GoalSamplingConfig &gs_config, int timesteps,
                   ValidityChecker &&validity_checker,
                   HeuristicFn &&heuristic_fn, NodeFactory &&node_factory,
  ...
  auto sampled_nodes = sample_and_create_nodes<NodeType>(
      start_pose, goal_pose, sample_config, timesteps,
