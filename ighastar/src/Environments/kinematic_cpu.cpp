...
float map_to_elev_cpu(float x, float y, const float *elev, int map_size_px,
                      float res_inv) {
  int img_X =
      std::min(std::max(static_cast<int>(x * res_inv), 0), map_size_px - 1);
  int img_Y =
      std::min(std::max(static_cast<int>(y * res_inv), 0), map_size_px - 1);
...
bool check_crop_cpu(const float x, const float y, const float cy,
                    const float sy, const float *map, const int map_size_px,
                    const float res, const float car_l2, const float car_w2) {
  ...
  for (int i = 0; i < patch_length_px; i++) {
    ...
    for (int j = 0; j < patch_width_px; j++) {
      ...
      if (px < 0 || px >= map_size_px * res || py < 0 ||
          py >= map_size_px * res) {
        return false;
      }

      int img_X = std::min(std::max(static_cast<int>(px * res_inv), 0),
                           map_size_px - 1);
      int img_Y = std::min(std::max(static_cast<int>(py * res_inv), 0),
                           map_size_px - 1);
  ...
  for (int k = 0; k < rollouts; k++) {
    ...
    for (int t = 1; t <= timesteps; t++) {
      ...
      get_footprint_z_cpu(fl, fr, bl, br, z, x, y, cy, sy, heightmap,
                          map_size_px, res_inv, car_l2, car_w2);
...
void check_validity_launcher_cpu(const float *bitmap, int map_size_px,
  ...
  for (int i = 0; i < n_states; i++) {
    ...
    result[i] = check_crop_cpu(x, y, cy, sy, bitmap, map_size_px, map_res,
                               car_l2, car_w2);
  }
}