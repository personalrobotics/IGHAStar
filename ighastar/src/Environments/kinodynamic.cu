...
__global__ void kinodynamic_kernel(
    ...
    float max_vert_acc, float max_theta, float gear_switch_time)
{
    ...
    for (int t = 1; t <= timesteps; t++) {
        ...
        ay = vx * wz - sr * GRAVITY;
        az = GRAVITY * cp * cr - vx * wy + wx*wx*car_w2; // assuming car width/2 ~ car cg height
        wz = curvature * vx;
    ...
    cost[k] = timesteps * dt + gear_switch_cost; // TODO: multiply by the time it takes to do the gear shift
