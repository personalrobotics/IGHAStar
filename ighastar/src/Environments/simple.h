...
size_t calc_hash(float *pose, const float *resolution) {
  ...
  for (int i = 0; i < n_dims; ++i) {
    boost::hash_combine(hash_val, std::hash<size_t>{}(static_cast<int64_t>(
...
public:
  ...
  void set_world(torch::Tensor world) {
    ...
    TORCH_CHECK(world.dim() == 2,
                "World tensor must be 2D (H x W) for simple environment");
    ...
    memcpy(costmap, world_contiguous.data_ptr<float>(),
