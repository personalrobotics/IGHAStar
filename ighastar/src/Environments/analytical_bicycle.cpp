...
inline float map_to_elev(float x, float y, const float *elev, int map_size_px,
                         float res_inv) {
...
void get_footprint_z(float *fl, float *fr, float *bl, float *br, float &z,
                     float x, float y, float cy, float sy, const float *elev,
                     int map_size_px, float res_inv, float car_l2,
                     float car_w2) {
...
bool check_crop(const float x, const float y, const float cy, const float sy,
  ...
  int img_X, img_Y, patch_length_px = int(2 * car_l2 * res_inv),
                    patch_width_px = int(2 * car_w2 * res_inv);
  ...
  for (int i = 0; i < patch_length_px; i++) {
    ...
    for (int j = 0; j < patch_width_px; j++) {
      ...
      valid = px > 0 && px < map_size_px * res && py > 0 &&
              py < map_size_px * res &&
              map_to_elev(px, py, map, map_size_px, res_inv) > 250.0;
      if (!valid) {
        return false;
      }
    }
...
bool check_validity(const float x, const float y, const float cy,
                    const float sy, torch::Tensor map_tensor,
                    const int map_size_px, const float res, const float car_l2,
                    const float car_w2) {
...
void kinematic_launcher(torch::Tensor state_tensor,
                        torch::Tensor controls_tensor,
                        torch::Tensor BEVmap_height_tensor,
                        torch::Tensor BEVmap_cost_tensor,
                        torch::Tensor valid_tensor, torch::Tensor cost_tensor,
                        float dt, int rollouts, int timesteps, int NX, int NC,
                        float throttle_to_wheelspeed, float steering_max,
                        int BEVmap_size_px, float BEVmap_res, float car_l2,
                        float car_w2) {
  ...
  // the state array is rollouts x NX. The initial value of all the states is
  // the same as the start node. omp_set_num_threads(rollouts); #pragma omp
  // parallel for
  ...
  for (int k = 0; k < rollouts; k++) {
    ...
    for (t = 0; t < timesteps; ++t) {
      ...
      x += dt * vx * cy;
      y += dt * vx * sy;
      if (valid[k] && !check_crop(x, y, cy, sy, BEVmap_cost, BEVmap_size_px,
                                  BEVmap_res, car_l2, car_w2)) {
        valid[k] = false;
        break;
      }
    }
...
void kinodynamic_launcher(torch::Tensor state_tensor,
                          torch::Tensor controls_tensor,
                          torch::Tensor BEVmap_height_tensor,
                          torch::Tensor BEVmap_cost_tensor,
                          torch::Tensor valid_tensor, torch::Tensor cost_tensor,
                          float dt, int rollouts, int timesteps, int NX, int NC,
                          float throttle_to_accel, float steering_max,
                          int BEVmap_size_px, float BEVmap_res, float car_l2,
  ...
  float initial_roll =
      std::atan2((fl[2] + bl[2]) - (fr[2] + br[2]), 4 * car_w2);
  float initial_pitch =
      std::atan2((bl[2] + br[2]) - (fl[2] + fr[2]), 4 * car_l2);
  ...
  for (int k = 0; k < rollouts; ++k) {
    ...
    for (t = 0; t < timesteps; ++t) {
      ...
      y += dt * (vx * (cp * sy) + vy * (sr * sp * sy + cr * cy) +
                 vz * (cr * sp * sy - sr * cy));
      valid[k] =
          valid[k] && check_crop(x, y, cy, sy, BEVmap_cost, BEVmap_size_px,
                                 BEVmap_res, car_l2, car_w2);
